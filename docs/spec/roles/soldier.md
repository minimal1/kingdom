...
# tmux 세션에 KINGDOM_TASK_ID, KINGDOM_RESULT_PATH export
...
> **출력 요구사항은 프롬프트에 포함하지 않는다.** `workspace/CLAUDE.md`가 결과 스키마와 환경변수(`KINGDOM_TASK_ID`, `KINGDOM_RESULT_PATH`) 참조를 지시하므로, 프롬프트 템플릿에서 별도로 출력 형식을 지시할 필요 없다. 상세: [roles/general.md — build_prompt](general.md#build_prompt)

## 결과 스키마

병사는 `workspace/CLAUDE.md`의 지시에 따라 환경변수 `KINGDOM_TASK_ID`와 `KINGDOM_RESULT_PATH`를 참조하여 Write 도구로 `state/results/{task-id}-raw.json`에 직접 결과를 저장한다. stdout은 `logs/sessions/{soldier-id}.json`으로, stderr는 `logs/sessions/{soldier-id}.err`로 분리 캡처되어 디버깅용으로 보존된다. stdout JSON에서 `session_id`를 추출하여 `state/results/{task-id}-session-id`에 저장한다.
...
병사 세션에서 동작하는 Claude Code hook. `install-general.sh`가 `~/.claude/settings.json`에 자동 등록한다.

### Stop Hook (`bin/lib/soldier/stop-hook.sh`)

Claude가 종료하려 할 때 결과 보고를 강제하는 게이트키퍼.

```
Claude 작업 완료 → 종료 시도
       │
       ├─ KINGDOM_TASK_ID 없음? → allow (일반 세션)
       ├─ stop_hook_active=true? → allow (재시도, 무한루프 방지)
       ├─ 결과 파일 존재? → allow (이미 보고함)
       └─ 결과 파일 없음? → block + reason 메시지
                              → Claude가 대화 맥락을 보고 직접 결과 JSON 작성
                              → 다시 종료 시도 → stop_hook_active=true → allow
```

LLM이 자신의 작업 맥락을 기반으로 `status`와 `summary`를 판단하여 결과를 작성한다. 기계적 추출이 아니므로 실패/스킵 등도 정확하게 보고된다.

### Heartbeat Hook (`bin/lib/soldier/heartbeat-hook.sh`)

PostToolUse hook. 도구 호출이 완료될 때마다 `state/results/{task-id}-heartbeat` 파일에 타임스탬프와 도구명을 기록한다.

```
2026-03-10T12:00:00Z Bash
```

내관의 `check_heartbeats()`가 이 파일의 mtime으로 병사의 마지막 활동 시각을 판단할 수 있다. 타임아웃 전에 "아직 활동 중인지 vs 멈춘 건지" 구분하는 용도.

## 스크립트 위치

- `bin/spawn-soldier.sh` — 병사 생성 + 실행
- `bin/lib/soldier/stop-hook.sh` — Stop hook (결과 보고 강제)
- `bin/lib/soldier/heartbeat-hook.sh` — Heartbeat hook (활동 추적)
