...
| `slack.app_mention` | Slack | 채널에서 @멘션 (Socket Mode) | normal | `evt-slack-mention-{ts_sanitized}` |
...
왕: 이벤트 소비
     ...
     │
     ├─ type: slack.channel.message | slack.app_mention
...
| Slack (mention) | `evt-slack-mention-{ts_sanitized}` | 사절 | ts 자연적 유일성 |
