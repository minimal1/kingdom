#!/usr/bin/env bash
# gen-jira 장군을 Kingdom에 설치
set -euo pipefail

KINGDOM_BASE_DIR="${KINGDOM_BASE_DIR:-/opt/kingdom}"
PACKAGE_DIR="$(cd "$(dirname "$0")" && pwd)"

# --- CC Plugin 설치: sunday@qp-plugin ---
if command -v claude &>/dev/null; then
  ...
  # sunday 플러그인 설치
  if [ ! -f "$SETTINGS" ] || ! jq -e '.enabledPlugins["sunday@qp-plugin"]' "$SETTINGS" &>/dev/null; then
    echo "Installing sunday plugin..."
    claude plugin install sunday@qp-plugin
  else
    echo "Plugin sunday@qp-plugin already installed."
  fi
else
  echo "WARN: claude CLI not found. Install CC plugins manually:"
  echo "  claude plugin marketplace add eddy-jeon/qp-plugin"
  echo "  claude plugin install sunday@qp-plugin"
fi
